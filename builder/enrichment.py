...
def enrich(config):
    ...
    route_lookup = (
        memberships.groupby(station_col)[route_col]
        .apply(list)
        .to_dict()
    )

    route_count_lookup = (
        memberships.groupby(station_col)
        .size()
        .to_dict()
    )
    ...
    stations["service_density"] = (
        stations["service_count"]
        .apply(classify_service_density)
    )

    stations["route_diversity_band"] = (
        stations["route_count"]
        .apply(classify_route_diversity)
    )
    ...
    stations["is_branch_line"] = (
        stations["route_groups"]
        .apply(lambda g: any(r in branch_groups for r in g))
    )
    ...
    stations["is_coastal"] = (
        stations["route_groups"]
        .apply(lambda g: any(r in coastal_groups for r in g))
    )
